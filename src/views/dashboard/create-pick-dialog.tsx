...
interface CreatePickDialogProps {
  disabled: boolean;
  revalidatePicks: KeyedMutator<ApiResponse>;
}

// TODO: Consider using reducer to handle state updates, revalidate and show notification
export default function CreatePickDialog({
  revalidatePicks,
  disabled,
}: CreatePickDialogProps) {
  ...
  const { isMutating: isSubmitting, trigger: createNewPick } = useSWRMutation(
    "/picks",
    (url) =>
      fetchApi(url, {
        body: JSON.stringify({
      ...
      }),
    {
      async onError(err) {
        ...
        toast(error.message || "Failed to create pick.");
      },
      onSuccess(data) {
        revalidatePicks();
        toast(data.message);
