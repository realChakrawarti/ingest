...
export default function EditCatalog({ catalogId }: { catalogId: string }) {
	...
	const handleDeleteSavedPlaylist = async (id: string) => {
		const deletePlaylist = savedPlaylists.find(
			(playlist) =>
				playlist.type === "playlist" && playlist.playlistId === id
		);
		if (!deletePlaylist) {
			return;
		}
	...
	const handleDeleteSavedSubreddit = async (id: string) => {
		...
		if (result.success) {
			toast(
				`${
					deleteSubreddit.type === "subreddit" &&
					deleteSubreddit.subredditTitle
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className="font-normal tracking-wide"
									variant="secondary">
									{savedPlaylists?.length} of 15 playlists
									added
								</Badge>
							</div>
							<PlaylistTable
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className="font-normal tracking-wide"
									variant="secondary">
									{savedSubreddits?.length} of 10 subreddits
									added
								</Badge>
							</div>
							<SubredditTable
