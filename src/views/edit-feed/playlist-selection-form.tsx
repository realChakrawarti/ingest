...
import type { ZFeedByID, ZFeedPlaylist } from "~/entities/feeds/models";
  ...
  const handleAddPlaylists = async () => {
    try {
      ...
      const playlistDetails = selectedPlaylists.map(
        (item: ChannelPlaylist): ZFeedPlaylist => ({
      ...
      if (result.success) {
        toast("Feed has been updated with new playlists.");
        revalidateFeed();
        resetTempData();
      } else {
...
function PlaylistItemCheckbox({
  ...
  const { selectedPlaylists } = useFeedStore();
