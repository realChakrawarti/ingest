...
import type { ZFeedByID, ZFeedPodcast } from "~/entities/feeds/models";
...
export default function AddPodcastDialog({
  ...
  const togglePodcastSelection = (podcast: any) => {
    ...
    if (podcastExists) {
      ...
      const podcastFormatted: ZFeedPodcast = {
  ...
  async function handleAddPodcast() {
    ...
    try {
      ...
      if (result.success) {
        toast("Feed has been updated with new podcasts.");
        revalidateFeed();
        resetTempData();
      } else {
...
function SearchDropdown({
  ...
  const { selectedPodcasts } = useFeedStore();
